> 注：原仓库地址为：https://github.com/Mutuduxf/CSharp-Coding-Standards
...
#### 19.业务对象尽量使用DateTimeOffset而不是DateTime，但是DTO或者PO对象由于序列化器/持久化的技术选型，可能并不支持DateTimeOffset（如protobuf不支持DateTimeOffset，MySql也不支持DateTimeOffset类型），这里要针对具体的技术进行选择。
